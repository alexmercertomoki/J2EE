		...
		$.ajax({
			...
			success: function(message) {
				if (message.type == "success") {
					window.location.href = "payment.jhtml?sn=" + message.content;
				} else {
					$.message(message);
					window.setTimeout(function() {
						window.location.reload(true);
				...
				<form id="orderForm" action="create.jhtml" method="post">
					...
					<table class="product">
						<tr>
							...
							<th>${message("shop.order.unitPrice")}</th>
						...
						<div class="total">
							[#if member.balance > 0]
								...
								<label for="useBalance">
									${message("shop.order.useBalance")} (${message("shop.order.balance")}: <strong>${currency(member.balance, true)}</strong>)
								</label>
							[/#if]
							[#if setting.isInvoiceEnabled && setting.isTaxPriceEnabled]
								<span class="hidden">${message("shop.order.tax")}: <em id="tax">${currency(order.tax, true)}</em>&nbsp;</span>
							[/#if]
