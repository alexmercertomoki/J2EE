...
$().ready(function() {
	...
	$.validator.addMethod("compareAmount", 
		...
		"${message("admin.promotion.compareAmount")}"
	);
	...
	$.validator.addMethod("divisor", 
		...
		"${message("admin.promotion.divisor")}"
	);
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<div class="tabContent">
			<table class="input">
				...
				<tr>
					<th>
						${message("Promotion.startPrice")}:
					</th>
					<td colspan="2">
						<input type="text" id="startPrice" name="startPrice" class="text" value="${promotion.startPrice}" maxlength="16" />
					</td>
				</tr>
				<tr>
					<th>
						${message("Promotion.endPrice")}:
					</th>
					<td colspan="2">
						<input type="text" name="endPrice" class="text" value="${promotion.endPrice}" maxlength="16" />
					</td>
				</tr>
				<tr>
					<th>
						${message("admin.promotion.price")}:
					</th>
					<td colspan="2">
						${message("admin.promotion.originalPrice")}
						<select id="priceOperator" name="priceOperator">
							[#list operators as operator]
								<option value="${operator}"[#if operator == promotion.priceOperator] selected="selected"[/#if]>${message("Promotion.Operator." + operator)}</option>
							[/#list]
						</select>
						<input type="text" name="priceValue" class="text" value="${promotion.priceValue}" maxlength="16" style="width: 50px;" />
					</td>
				</tr>
				<tr>
					<th>
						${message("admin.promotion.point")}:
					</th>
					<td colspan="2">
						${message("admin.promotion.originalPoint")}
						<select id="pointOperator" name="pointOperator">
							[#list operators as operator]
								<option value="${operator}"[#if operator == promotion.pointOperator] selected="selected"[/#if]>${message("Promotion.Operator." + operator)}</option>
							[/#list]
						</select>
						<input type="text" name="pointValue" class="text" value="${promotion.pointValue}" maxlength="16" style="width: 50px;" />
					</td>
				</tr>
				<tr>
					<th>
