							...
							<tr>
								...
								<td>
									<select id="type" name="type">
										[#list types as paymentMethod]
											<option value="${paymentMethod}">${message("Payment.Type." + paymentMethod)}<\/option>
										[/#list]
									<\/select>
								<\/td>
								<th>
				...
				onShow: function() {
					...
					$.validator.addMethod("balance", 
						function(value, element, param) {
							return this.optional(element) || $type.val() != "deposit" || parseFloat(value) <= parseFloat(param);
						},
				...
				[@compress single_line = true]
					...
					<form id="refundsForm" action="refunds.jhtml" method="post">
						...
						<table class="input">
							...
							<tr>
								<th>
									${message("Refunds.type")}:
								<\/th>
								<td>
									<select name="type">
										[#list refundsTypes as refundsType]
											<option value="${refundsType}">${message("Refunds.Type." + refundsType)}<\/option>
										[/#list]
									<\/select>
								<\/td>
								<th>
	...
	<table class="input tabContent">
		...
		[#list order.refunds as refunds]
			<tr>
				...
				<td>
					${message("Refunds.Type." + refunds.type)}
				</td>
				<td>
