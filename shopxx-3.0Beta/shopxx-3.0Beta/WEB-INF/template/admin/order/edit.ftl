...
$().ready(function() {
	...
	// 计算
	setInterval(function() {
		if (isLocked || !$inputForm.valid()) {
			return false;
		}
		$inputForm.find(":input:not(#productSn)").each(function() {
			var $this = $(this);
			var value;
			if ($this.is(":checkbox")) {
			...
			} else if (recalculated || $this.data("value") != value) {
				$.ajax({
					url: "calculate.jhtml",
					type: "POST",
					...
					success: function(data) {
						if (data.message.type == "success") {
							...
							if ($this.hasClass("orderItemPrice") || $this.hasClass("orderItemQuantity")) {
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input tabContent">
			...
			<tr>
				...
				<td>
					<input type="text" name="discount" class="text" value="${order.discount}" maxlength="16" />
				</td>
				<th>
					${message("Order.point")}:
				</th>
